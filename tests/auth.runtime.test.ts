...
const users = [
  ...
  { id: 'pimpinan-1', username: 'pimpinantest', password: 'PassPimpinan123!', nama: 'Pimpinan Test', role: 'Pimpinan', idSantri: '' },
...
test('all five roles receive usable custom tokens with correct claims', async () => {
  for (const user of users) {
...
test('secure admin account lifecycle enforces staff boundary and never stores plaintext passwords', async () => {
  ...
  const pimpinan = users.find((user) => user.role === 'Pimpinan')!;
  ...
  const pimpinanCreateDenied = await call(adminUsersHandler, {
    method: 'POST',

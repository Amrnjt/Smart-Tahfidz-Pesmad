...
Wassalamu'alaikum Warahmatullahi Wabarakatuh.`;
  ...
  return (
    <div className="p3-management-page p3-santri-management space-y-6">
                  ...
                  filteredUsers.map((u) => {
                    ...
                    return (
                      <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                        <td className="py-3 px-3.5 font-bold text-slate-800">{u.nama}</td>
                        <td className="py-3 px-3.5 font-mono text-slate-700">{isSuperadmin ? 'Dirahasiakan' : u.username}</td>
                        <td className="py-3 px-3.5 font-mono text-slate-500">{isSuperadmin ? 'Dirahasiakan' : u.password}</td>
                        <td className="py-3 px-3.5">{roleBadge}</td>
                        <td className="py-3 px-3.5 font-mono text-slate-500">{u.idSantri || '-'}</td>
                        <td className="py-3 px-3.5 text-center">
                          {isSuperadmin ? (
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-50 text-amber-900 text-xs font-bold border border-amber-200 shadow-2xs"><Shield className="w-3.5 h-3.5 text-amber-600" /> Akun Terproteksi</span>
                          ) : (
                            <div className="flex items-center justify-center gap-1.5">
                              <button type="button" onClick={() => handleCopyUserCredentials(u)} className="p-1.5 text-slate-600 bg-slate-100 hover:bg-emerald-50 hover:text-emerald-800 rounded-lg transition cursor-pointer flex items-center gap-1 text-xs font-semibold border border-slate-200"><Copy className="w-3.5 h-3.5" /><span className="hidden sm:inline">Salin</span></button>
                              <button type="button" onClick={() => handleShareUserCredentials(u)} className="p-1.5 px-2 text-emerald-800 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition cursor-pointer flex items-center gap-1 text-xs font-semibold border border-emerald-200"><Share2 className="w-3.5 h-3.5 text-emerald-700" /><span className="hidden sm:inline">Bagikan</span></button>
                              <button type="button" onClick={() => handleOpenEditUser(u)} className="p-1.5 px-2 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition cursor-pointer flex items-center gap-1 text-xs font-semibold border border-emerald-200"><SquarePen className="w-3.5 h-3.5 text-emerald-700" /><span>Edit</span></button>
                              <button type="button" onClick={() => setUserToDelete(u)} aria-label={`Hapus akun ${u.nama}`} className="min-h-11 min-w-11 inline-flex items-center justify-center text-rose-500 hover:bg-rose-50 rounded-lg transition cursor-pointer"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                          )}
                        </td>
                      </tr>
      ...
      {santriToEdit && (
        <div ref={editSantriDialogRef} className="ui-dialog-overlay" role="dialog" aria-modal="true" aria-label="Edit data santri" tabIndex={-1}>
          <div className="ui-dialog-panel max-w-md p-5 sm:p-6 space-y-5">
            <div className="flex items-start justify-between gap-3 pb-4 border-b border-slate-200">
              <h4 className="font-extrabold text-slate-800 text-base flex items-center gap-2"><SquarePen className="w-5 h-5 text-emerald-700" />Edit Data Santri ({santriToEdit.idSantri})</h4>
              <button onClick={() => setSantriToEdit(null)} aria-label="Tutup dialog edit santri" className="ui-dialog-close cursor-pointer text-lg">✕</button>
            </div>
            <form onSubmit={handleSaveEditSantri} className="space-y-3.5">
              <input type="text" required value={editSantriNama} onChange={(e) => setEditSantriNama(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              <select value={editSantriKelas} onChange={(e) => setEditSantriKelas(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium"><option value="Tahfidz">Tahfidz</option><option value="Binnadzor A">Binnadzor A</option><option value="Binnadzor B">Binnadzor B</option><option value="Jilid">Jilid</option><option value="Kelas Istimewa">Kelas Istimewa</option></select>
              <input type="text" value={editSantriTarget} onChange={(e) => setEditSantriTarget(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              <input type="text" value={editSantriWaliNama} onChange={(e) => setEditSantriWaliNama(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              <input type="text" value={editSantriWaliKontak} onChange={(e) => setEditSantriWaliKontak(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              <div className="ui-dialog-footer"><button type="button" onClick={() => setSantriToEdit(null)} className="ui-control">Batal</button><button type="submit" disabled={isSaving} className="ui-control bg-emerald-800 text-white"><Save className="w-4 h-4" />Simpan Perubahan</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {userToEdit && (
        <div ref={editUserDialogRef} className="ui-dialog-overlay" role="dialog" aria-modal="true" aria-label="Edit akun pengguna" tabIndex={-1}>
          <div className="ui-dialog-panel max-w-md p-5 sm:p-6 space-y-5">
            <div className="flex items-start justify-between gap-3 pb-4 border-b border-slate-200"><h4 className="font-extrabold text-slate-800 text-base flex items-center gap-2"><SquarePen className="w-5 h-5 text-emerald-700" />Setting Role & Edit Akun Pengguna</h4><button onClick={() => setUserToEdit(null)} aria-label="Tutup dialog edit akun" className="ui-dialog-close cursor-pointer text-lg">✕</button></div>
            <form onSubmit={handleSaveEditUser} className="space-y-3.5">
              <input type="text" required value={editNama} onChange={(e) => setEditNama(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              {userToEdit.role !== 'Superadmin' && <><input type="text" required value={editUsername} onChange={(e) => setEditUsername(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" /><input type="text" required value={editPassword} onChange={(e) => setEditPassword(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" /></>}
              <select value={editRole} onChange={(e) => setEditRole(e.target.value as UserRole)} className="w-full p-2.5 bg-emerald-50 border border-emerald-300 rounded-xl text-xs font-bold text-emerald-950">
              ...
              {(editRole === 'Wali' || editRole === 'Santri') && <select value={editIdSantri} onChange={(e) => setEditIdSantri(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium"><option value="">-- Pilih ID Santri Terkait --</option>{santriList.map((s) => <option key={s.idSantri} value={s.idSantri}>{s.idSantri} - {s.namaSantri} ({getClassGroup(s.kelas)})</option>)}</select>}
              <div className="ui-dialog-footer"><button type="button" onClick={() => setUserToEdit(null)} className="ui-control">Batal</button><button type="submit" disabled={isSaving} className="ui-control bg-emerald-800 text-white"><Save className="w-4 h-4" />Simpan Perubahan Role & Akun</button></div>
            </form>
          </div>
        </div>
      )}

      {santriToDelete && <div ref={deleteSantriDialogRef} className="ui-dialog-overlay" role="dialog" aria-modal="true" tabIndex={-1}><div className="ui-dialog-panel max-w-md p-5 sm:p-6 space-y-5"><h4 className="font-extrabold">Hapus Data Santri?</h4><label className="flex items-start gap-2.5 text-xs"><input type="checkbox" checked={deleteWithHistory} onChange={(e) => setDeleteWithHistory(e.target.checked)} />Hapus juga seluruh riwayat dan akun login wali & santri ini.</label><div className="ui-dialog-footer"><button onClick={() => setSantriToDelete(null)} disabled={isDeleting}>Batal</button><button onClick={handleDeleteSantri} disabled={isDeleting}>Ya, Hapus Santri</button></div></div></div>}
      {userToDelete && <div ref={deleteUserDialogRef} className="ui-dialog-overlay" role="dialog" aria-modal="true" tabIndex={-1}><div className="ui-dialog-panel max-w-md p-5 sm:p-6 space-y-5"><h4 className="font-extrabold">Hapus Akun Pengguna?</h4><p>{userToDelete.nama} · {userToDelete.role}</p><div className="ui-dialog-footer"><button onClick={() => setUserToDelete(null)} disabled={isDeleting}>Batal</button><button onClick={handleDeleteUser} disabled={isDeleting}>Ya, Hapus Akun</button></div></div></div>}

      {showAddModal && <div ref={addSantriDialogRef} className="ui-dialog-overlay" role="dialog" aria-modal="true" tabIndex={-1}><div className="ui-dialog-panel max-w-md p-5 sm:p-6 space-y-5"><h4 className="font-extrabold">Tambah Data Santri Baru</h4><form onSubmit={handleAddSantri} className="space-y-3.5"><input value={newId} onChange={(e) => setNewId(e.target.value)} /><input required value={newNama} onChange={(e) => setNewNama(e.target.value)} /><input value={newKelas} onChange={(e) => setNewKelas(e.target.value)} /><input value={newTarget} onChange={(e) => setNewTarget(e.target.value)} /><input value={newWaliNama} onChange={(e) => setNewWaliNama(e.target.value)} /><input value={newWaliKontak} onChange={(e) => setNewWaliKontak(e.target.value)} /><input value={defaultPassword} onChange={(e) => setDefaultPassword(e.target.value)} /><button type="submit" disabled={isSaving}>Simpan Data Santri</button></form></div></div>}
      ...
      {showAddUserModal && (
        <div ref={addUserDialogRef} className="ui-dialog-overlay" role="dialog" aria-modal="true" aria-label="Tambah akun pengguna" tabIndex={-1}>
          <div className="ui-dialog-panel max-w-md p-5 sm:p-6 space-y-5">
            <div className="flex items-start justify-between gap-3 pb-4 border-b border-slate-200"><h4 className="font-extrabold text-slate-800 text-base flex items-center gap-2"><Shield className="w-5 h-5 text-emerald-700" />Tambah Akun Pengguna Baru</h4><button onClick={() => setShowAddUserModal(false)} className="ui-dialog-close cursor-pointer text-lg">✕</button></div>
            <form onSubmit={handleAddUser} className="space-y-3.5">
              <input type="text" required value={newUserNama} onChange={(e) => setNewUserNama(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              <input type="text" required value={newUsername} onChange={(e) => setNewUsername(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              <select value={newUserRole} onChange={(e) => setNewUserRole(e.target.value as UserRole)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium"><option value="Superadmin">Superadmin</option><option value="Ustadz">Ustadz</option><option value="Wali">Wali Santri</option><option value="Santri">Santri (View-Only)</option></select>
              <input type="password" required value={newUserPassword} onChange={(e) => setNewUserPassword(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium" />
              {(newUserRole === 'Wali' || newUserRole === 'Santri') && <select value={newUserIdSantri} onChange={(e) => setNewUserIdSantri(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium"><option value="">-- Pilih ID Santri --</option>{santriList.map((s) => <option key={s.idSantri} value={s.idSantri}>{s.idSantri} - {s.namaSantri}</option>)}</select>}
